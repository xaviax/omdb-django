...
def fill_movie_details(movie):
    ...
    if movie.is_full_record:
      logger.warning(
        "This movie is already a full record"
      )
      return
...
def search_and_save(search):
  ...
  if it hasn't been searched in the past 24 hours.
...
# If you forgot where to read or start from then its after the above func in the guide
